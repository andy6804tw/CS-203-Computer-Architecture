...
/**
 * Reservation Station 與StoreBuffer&LoadBuffer共用
 */
...
public class tomasulo {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            for (int i = 0; i < loadMount; i++) {
                if (loadBuffer[i].busy == 1) {
                    ...
                    if (ins.executed != 0 && ins.written == 0) {
                        ins.written = clock;
                        // 取得整數暫存器的數值
                        double iRegisterValue = Integer.parseInt(iRegister.get(ins.rt));

                        // 廣播Load更新storeBuffer
                        for (int j = 0; j < loadMount; j++) {
                            if (storeBuffer[j].Qj.equals("Load" + i)) {
                                storeBuffer[j].Qj = "";
                                storeBuffer[j].Vj = iRegisterValue + "";
                            }
                        }
                        // 廣播Load更新adder
                        for (int j = 0; j < addMount; j++) {
                            if (adder[j].Qj.equals("Load" + i) && adder[j].remain == 0) {
                                adder[j].Qj = "";
                                adder[j].Vj = iRegisterValue + "";
                            }
                            if (adder[j].Qk.equals("Load" + i) && adder[j].remain == 0) {
                                adder[j].Qk = "";
                                adder[j].Vk = iRegisterValue + "";
                            }
                        }
                        // 廣播Load更新multiplier
                        for (int j = 0; j < mulMount; j++) {
                            if (multiplier[j].Qj.equals("Load" + i)) {
                                multiplier[j].Qj = "";
                                multiplier[j].Vj = iRegisterValue + "";
                            }
                            if (multiplier[j].Qk.equals("Load" + i)) {
                                multiplier[j].Qk = "";
                                multiplier[j].Vk = iRegisterValue + "";
                            }
                        }
                    }
                    // 清空Station
                ...
                if (multiplier[i].busy == 1) {
                    ...
                    if (ins.executed != 0 && ins.written == 0) {
                        ...
                        if (ins.opcode.equals("MUL.D"))
                            broadcastValue = Double.parseDouble(multiplier[i].Vj)
                                    * Double.parseDouble(multiplier[i].Vk);
            ...
            /** 執行 */
            /**
             * Issue 當有資源可利用時(busy=0)，檢查下一個指令是否可以被Issue loadBuffer[2]=> L.D stroeBuffer[2]=>
             * S.D adder[3]=> ADD.D、SUB.D multiplier[2]=> MUL.D、DIV.D
             */
        ...
        writeOutput();
        System.out.println("End: " + totalClock);
        scn.nextLine();
    ...
    public static void showInfo() {
        ...
        for (int i = 0; i < instructionList.size(); i++) {
            ...
            System.out.printf("%-14s%-10s%-14s%-14s%-10s\n", ins.opcode, ins.issue, ins.execution, ins.executed,
                    ins.written);
        }
        System.out.println("\n-----------------------------------------------------------------------");
        ...
        for (int i = 0; i < addMount; i++) {
            System.out.printf("%-5s%-8s%-8s%-8s%-8s%-8s%-8s%-8s\n", adder[i].remain, "Add" + (i + 1), adder[i].opcode,
                    adder[i].Vj, adder[i].Vk, adder[i].Qj, adder[i].Qk, adder[i].busy);
        }
        for (int i = 0; i < mulMount; i++) {
            System.out.printf("%-5s%-8s%-8s%-8s%-8s%-8s%-8s%-8s\n", multiplier[i].remain, "Mul" + (i + 1),
                    multiplier[i].opcode, multiplier[i].Vj, multiplier[i].Vk, multiplier[i].Qj, multiplier[i].Qk,
                    multiplier[i].busy);
        }
        System.out.println("\n-----------------------------------------------------------------------");
        ...
        for (int i = 0; i < loadMount; i++) {
            System.out.printf("%-8s%-8s%-8s%-8s\n", "Load" + (i + 1), loadBuffer[i].busy, loadBuffer[i].Vj,
                    loadBuffer[i].Qj);
        }
        System.out.println("\n-----------------------------------------------------------------------");
        ...
        System.out.printf("%-8s%-8s%-8s%-8s\n", "Name", "Busy", "Vj", "Qj");
        for (int i = 0; i < storeMount; i++) {
            System.out.printf("%-8s%-8s%-8s%-8s\n", "Store" + (i + 1), storeBuffer[i].busy, storeBuffer[i].Vj,
                    storeBuffer[i].Qj);
        }
        System.out.println("=======================================================================");
    }
    ...
    public static void save() {
        ...
        str += String.format("%-5s%-8s%-8s%-8s%-8s%-8s%-8s%-8s\n", "Time", "Name", "OP", "Vj", "Vk", "Qj", "Qk",
                "Busy");
        for (int i = 0; i < addMount; i++) {
            str += String.format("%-5s%-8s%-8s%-8s%-8s%-8s%-8s%-8s\n", adder[i].remain, "Add" + (i + 1),
                    adder[i].opcode, adder[i].Vj, adder[i].Vk, adder[i].Qj, adder[i].Qk, adder[i].busy);
        }
        for (int i = 0; i < mulMount; i++) {
            str += String.format("%-5s%-8s%-8s%-8s%-8s%-8s%-8s%-8s\n", multiplier[i].remain, "Mul" + (i + 1),
                    multiplier[i].opcode, multiplier[i].Vj, multiplier[i].Vk, multiplier[i].Qj, multiplier[i].Qk,
